...
In the app the question is yes, no, or comment: `→` or `Enter` keeps, `←` disposes, and typing writes a note into the always-focused field under the card. `1` to `4` pick extra actions, `↓` moves the card to the bottom of the deck, `⌘Z` undoes; with text in the note, `⌘` plus the same keys. The agent's suggestion is an amber mark on the card's edge and a dot beside the suggested outcome, never a key. Cards never expand: a body is at most four lines, so a hard case gets a note or a deferral rather than more reading. Every control names its key on hover. Cards also drag with the mouse and swipe with two fingers on a trackpad; a swipe commits when the fingers lift, so pulling back cancels.
